...
# Force GCC compiler on Windows if available
# Note: Commented out due to distutils.msvccompiler deprecation in Python 3.12+
# if sys.platform.startswith("win") and "CC" in os.environ and "gcc" in os.environ["CC"]:
#     from distutils import msvccompiler
#     import distutils._msvccompiler
#     
#     # Override MSVC compiler detection
#     def my_spawn(cmd):
#         import subprocess
#         subprocess.check_call(cmd)
#     
#     # Monkey patch to use GCC
#     msvccompiler.MSVCCompiler.spawn = my_spawn
...
# Window compilation: flags are for Visual C++ or GCC

elif sys.platform.startswith("win"):  # Windows
    # Check if GCC is available (e.g., from MSYS2)
    if "CC" in os.environ and "gcc" in os.environ["CC"]:
        print("Using GCC compiler on Windows")
        extra_compile_args = ["-w", "-std=c++11", "-O3", "-fopenmp"]
